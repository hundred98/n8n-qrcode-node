...
### 功能概述
通过二维码传输任意数据，支持JSON和纯文本格式，并提供qrKey验证机制确保数据安全。

### 配置参数

#### 输入配置
- **Data Source**: 输入源（Binary Field 或 URL）
- **Input Binary Field Name**: 二进制字段名称（当选择Binary Field时）
- **Image URL**: 图像URL（当选择URL时）
- **QR Key**: 用于验证的密钥（可选）
- **Output Format**: 输出格式（Text 或 JSON）

### qrKey验证机制

qrKey是一个安全特性，用于确保只有预期的接收方可以访问二维码中的数据：

1. 在节点配置中设置QR Key值
2. 二维码中必须包含相同的qrKey字段
3. 只有当两者匹配时，才会输出二维码中的其他数据
4. 如果未设置QR Key，则跳过验证直接输出数据

### 输出结果
- `success`: 操作是否成功
- `data`: 解析后的数据（如果是JSON格式）
- 其他自定义字段：根据二维码内容动态生成
...
1. 该功能可以从二维码中读取任意数据
2. 支持JSON格式和纯文本格式
3. 如果是JSON格式，会自动解析并展开为对象
4. qrKey字段用于数据验证，不会包含在输出结果中
